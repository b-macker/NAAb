...
### 5. Python Polyglot Multi-Line Dictionaries (Now Supported!)

**Problem:** Older documentation/features claimed multi-line Python dictionaries in polyglot blocks caused syntax errors.

**Solution:** The interpreter's Python adapter has been updated to correctly handle multi-line dictionaries as implicit return values or within `return` statements.
...
// This was previously marked as BAD but is now fully supported.
// The Python executor automatically wraps code containing `return` statements
// or multi-line expressions to ensure they execute correctly.
```
...
### Adding a New Stdlib Function (Example: `string.reverse()`)

The `string.reverse()` function has already been added to the standard library. This section demonstrates the process for *how you would* add a new stdlib function to NAAb, using `string.reverse()` as an example.
