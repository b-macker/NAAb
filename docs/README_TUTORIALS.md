...
**Duration:** 15 minutes
**Covers:**
- Block Assembly System (585+ blocks and growing!)
- **IMPORTANT:** First-time setup (indexing blocks)
...
```bash
cd ~/.naab/language/build
# Index blocks once before first use:
...
- **FIRST:** Index blocks with `./naab-lang blocks index ~/.naab/language/blocks/library`
...
```naab
# Search for blocks (in terminal)
# $ ./naab-lang blocks search "validate email"

# Use a block in code
use BLOCK-PY-09145 as validate_email
let is_valid = validate_email("alice@example.com")
print(is_valid)  # true
```
...
Documentation:
```
~/.naab/docs/AI_ASSISTANT_GUIDE.md  # Complete reference
