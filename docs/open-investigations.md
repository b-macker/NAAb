...
| B8 | **13 of 20 BSD event-type names are silent no-ops when written in the documented `UPPERCASE_UNDERSCORE` form.** `matchesStep` accepts a step name if `normalizeEventTypeName(matcher)` (lowercase, `_`→`.`) equals `eventTypeToString(event.type)`. Those two disagree for most types, so the enum spelling matches nothing and no warning is printed. Confirmed empirically for `ENV_READ` (`env.read` vs the actual **`env.get`**); the rest is derived from the two functions and still wants a fixture per type. **Works:** `FILE_READ`, `FILE_WRITE`, `AGENT_SEND`, `AGENT_RESPONSE`, `ENCODE`, `DECODE`, `PROCESS_EXEC`. **Silently dead:** `ENV_READ`, `ENV_WRITE`, `NET_CONNECT`, `SHELL_EXEC`, `TAINT_VIOLATION`, `TAINT_SANITIZED`, `CONFIG_RELOAD`, `CHECK_FAILED`, `TOOL_CALL`, `TOOL_RESULT`, `TOOL_ERROR`, `TOOL_BLOCKED`, `CODEGEN_EXEC`. CLAUDE.md states both formats are accepted and gives `AGENT_SEND` as its example — one of the seven that happen to work, which is why the claim reads as verified. | open | **This row previously said user-defined patterns could not be made to fire at all. That was wrong** and is corrected rather than deleted: patterns fire correctly in a plain script, in agent context, and with an intervening event. The original conclusion stacked two errors — the wrong step name above, and then counting `BSD_MATCH` telemetry, which the firing path does not write (B9). Fix should be a warning naming the valid spellings, not renaming the enums: renaming would break the seven forms that work today. Needs a positive control (`AGENT_SEND`, which does work) or a table of non-firing patterns proves only that nothing fired. |
| B9 | **The pre-execution BSD path enforces without evidence or budget.** There are two enforce sites for `behavioral_sequences.<name>`: `checkBehavioralSequence` (`governance_engine.cpp:7351`) writes a `BSD_MATCH` telemetry event and calls `consumeRiskBudget(…, 1)`; `checkPreExecution` (`:7901`) does **neither**. `checkPreExecution` handles `agent.send`, `file.read`/`write`, `http.*`, `crypto` encode, `process.*` and `env.get`/`list` (`vm.cpp:1855-1870`), so most real matches take the silent path. Measured: an 8-turn agent run where the pattern fired on **every** turn produced one stderr warning, one `RuleViolation`, and **zero** `BSD_MATCH` events — instrumentation confirmed all 8 firings came from `checkPreExecution` and `checkBehavioralSequence` fired 0 times. The `BSD_MATCH` write carries a comment saying it exists so ADVISORY matches stay visible, which its sibling path defeats. | open | Emitting `BSD_MATCH` from `checkPreExecution` is additive and should carry a field distinguishing the two paths. The risk-budget asymmetry is a **decision**, not obviously a bug — a pre-execution block prevents the action, so charging budget for it is arguable either way — but the current state looks unconsidered rather than chosen. Two further observations from the same instrumentation, neither investigated: (a) at ADVISORY the pre-check calls `recordEvent` and then execution continues, so the VM's normal emit records the **same action twice** (seen as two sequence ids for one logical send), which shifts `max_gap` arithmetic; (b) the pre-check detail for `agent.send` is the serialized handle dict, so a truncated `__nonce` prefix reaches stderr and telemetry details where the config name was clearly intended. |
...
| D9 | **`test_prescan_canaries.sh` can leave invalid C++ in the working tree, and its revert helper fails silently.** Observed 2026-08-30: after a full suite run that reported `test_prescan_canaries.sh: ALL PASSED`, `src/runtime/governance_config.cpp` still contained C8's payload — `void canary_p2_int(nlohmann::json j) { int x = j.get<int>(); }` — injected *inside* `~GovernanceEngine()`, where a function definition is invalid C++ and breaks the next build with a confusing error far from its cause. **Three distinct problems, all verified by reading the test:** (a) `revert()` runs `git checkout -- "$relfile"` with NO `cd` to the repo root and swallows errors via `2>/dev/null`, so off-root it fails silently — note `cleanup()` DOES `cd` first, i.e. the helper called on every canary is the unsafe one and the EXIT trap is the only safe path; (b) an EXIT trap does not run on SIGKILL, which is exactly how container resets kill long suites, so the documented "critical if killed mid-test" guarantee does not hold for the kill that actually happens; (c) **C7 ("File correctly reverted after injection") runs BEFORE C8, C9 and C10 inject**, so the only revert-verification gate cannot see the last three injections — which is why C7 passed on a run that left the tree dirty. **Second hazard, independent of the leftover:** `revert()`/`cleanup()` use `git checkout --` on `src/runtime/governance_config.cpp` and `src/interpreter/interpreter.cpp`, so ANY uncommitted work in those two files is silently destroyed by running the suite. | open | **Not determined: which of (a) or (b) produced this specific instance.** Both fit the evidence and I could not separate them — recorded as an open question rather than resolved to the tidier story. Candidate fixes, none traced: `cd "$LANG_DIR"` inside `revert()` and drop the error suppression so a failed revert is loud; move the C7-style verification to run after the LAST injection (or assert clean in the EXIT trap); and either stash-and-restore instead of `git checkout --`, or refuse to run when those files are dirty. Same family as D1/D4 — harness that reports success while leaving the tree wrong. |
