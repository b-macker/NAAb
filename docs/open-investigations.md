...
| A20 | **`checkPrivilegeEscalation` regexes miss octal SUID and hyphen-less `su` — a source-regex completeness gap (and source-text-only).** `block_su` = `\bsu\s+-` and `block_chmod_suid` = `chmod\s+[ugo]*s`. | **VERIFIED (partly — reporter overclaimed)** | Traced. `su root` (no hyphen) and octal `chmod 4755` bypass, as reported. BUT the reporter also listed `su -l` and `su -c` as bypasses — those DO match `\bsu\s+-` (the `-` follows), so that part is WRONG; only the hyphen-less and octal forms escape. Same class as F8a (regex completeness) AND, being source-text-only, also bypassed wholesale by native `process.run("su",...)` per A19/F17. Fix direction: match octal SUID (`chmod\s+[0-7]*[4-7][0-7]{3}`) and `su` without requiring a trailing hyphen; but recognise source-text is the weaker mechanism — the real fix is gating at the exec call site. Not fixed here. |
