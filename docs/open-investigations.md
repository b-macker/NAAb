...
| B6 | **Action diversity after turn 0 is invisible to CDD.** `ev.turn` is stamped from `current_agent_turn_` (`governance_engine.cpp:6415`), which is written *only* by `setAgentContext()` (6432/6438) at the START of each send. An event raised BETWEEN send N and send N+1 therefore carries turn N, whose bucket `checkContextDrift()` already consumed during send N. In a script with no registered tools the only live bucket is **turn 0's**. Measured, 25-turn arms differing only in *when* the script does file/env/crypto work: varied ops before EVERY send fires S5 on exactly the same turns as two ops once at startup (an agent that never stopped being diverse is charged identically to one that did nothing since turn 0), and varied ops introduced from turn 10 leave **every** CDD turn byte-identical to a control that never did any of them — not merely uncredited but wholly invisible, `scope_creep` included. | **open — undocumented** | Pinned by `test_vocab_baseline.sh` VC-05/VC-06. **A caveat on the evidence:** `scope_creep` (S3) silence is *not* an independent control here — S3 needs ≥3 turns of history and turn 0 is the only live bucket, so in a no-tools fixture it is structurally unable to fire either way. The mechanism trace (single writer, stamp source, already-consumed bucket) is what carries this, not the S3 observation. Affects every signal that reads `turn_events`, not only S5; S3's reachability from ordinary scripts should be re-checked as part of any fix. |
