...
| A3c | **The bounded-healing ledger can be over-counted without detection.** `test_bounded_healing.sh` degradation G2 inflated `coherence_damage_total` by +10/turn and no gate failed: BH-03 asserts `healed <= damage`, which an inflated ledger satisfies trivially while permitting more healing than was ever lost. | open | Add a consistency gate: `damage_total` should reconcile with coherence actually lost — roughly `(1 - min_coherence_lifetime) + healed_total`. The bound is verified in the safety-relevant direction and unverified in the direction where the engine mis-books its own damage. |
