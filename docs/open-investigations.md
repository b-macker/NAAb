...
| B8 | **A user-defined `behavioral_sequences.patterns` entry could not be made to fire.** While building the EF-05 gate, a two-step pattern `["env.get", "agent.send"]` with `max_gap: 5` produced **zero** `BSD_MATCH` events on a fixture that raises exactly that pair every turn, under both feeds. Config parsed correctly (verified in the written `govern.json`), the step strings match what `eventTypeToString` returns (`ENV_READ` → **`"env.get"`**, NOT `"env_read"` — a step written as `ENV_READ` normalizes to `env.read` and matches nothing, the same name-divergence trap CLAUDE.md documents for CDD signal keys), `configure()` installs config patterns and only falls back to defaults when the list is empty, and no warning was printed. | open | **Not investigated further — deliberately out of scope** for the change that surfaced it, and recorded rather than absorbed. EF-05 was reshaped into a cross-subsystem equality check whose control can pass, instead of shipping a gate whose control cannot. Worth checking before trusting ANY custom pattern: if this reproduces, every config carrying hand-written `patterns` has been running with silent no-ops — and because `configure()` only builds defaults when the list is EMPTY, such a config also loses the built-in patterns it would otherwise have had. Needs a positive control (a pattern known to fire) before concluding anything: the first two explanations here — wrong config key, wrong step name — were both wrong. |
