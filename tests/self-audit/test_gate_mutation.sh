...
# A fresh worktree does NOT populate submodules -- `git worktree add` checks out
# tracked files only, so external/abseil-cpp, fmt, json, spdlog and tomlplusplus
# are EMPTY directories. CMakeLists.txt add_subdirectory()s four of them, so
# cmake configure fails and the harness aborts before probing anything. That is
# why this had never produced a verdict in a fresh container: the instrument
# could not start. It fails loudly rather than silently, which is the one mercy.
#
# `submodule update --init` inside the worktree reuses the main repo's
# .git/modules store rather than re-cloning -- measured at 33s here, not a
# network fetch of five upstreams.
echo "  populating submodules in the worktree..."
if ! git -C "$WT" submodule update --init --recursive >/dev/null 2>&1; then
    echo -e "  ${RED}submodule init failed in the worktree -- cmake would fail on${NC}"
    echo -e "  ${RED}external/abseil-cpp. Refusing to report.${NC}"; exit 1
fi
...
# Pinned BEFORE any probe is committed. Every inject() restores the engine from
# this commit, so gate N is probed against pristine source rather than against
# the accumulated probes of gates 1..N-1.
BASE_COMMIT="$(git -C "$WT" rev-parse HEAD)"
echo ""

# BUG 4, and the one that would have corrupted the first real sweep. This line
# used to read `git checkout -q -- <file>`, which restores from the INDEX -- and
# commit_probe() has just written the previous gate's probe into that index. So
# each gate was probed ON TOP OF every earlier gate's probe: by gate 50, fifty
# gates were neutered at once. Attribution is destroyed, and the error runs in
# the FALSE PROTECTED direction this file's own header calls the worst verdict
# it can produce -- gate N reads PROTECTED because gate 3's absence broke a test.
#
# Invisible in --gate mode, where the only prior probe is the inert sanity rule.
# --gate is the only mode that had ever been run, which is exactly why a harness
# rebuilt three times over still carried this.
#
# Restoring from BASE_COMMIT (pinned before the sanity probe) makes each gate
# independent again.
inject() {
    git -C "$WT" checkout -q "$BASE_COMMIT" -- src/runtime/governance_engine.cpp 2>/dev/null
...
for rule in $TARGETS; do
    ...
    # CONTROL for the accumulation bug above. Exactly one probe may be live, or
    # this gate's verdict is really a verdict about a set of gates. Asserted per
    # gate rather than trusted: the bug it guards survived three rebuilds of this
    # harness because the mode that exposes it was never the mode being tested.
    nprobe=$(grep -c "$MARKER" "$ENGINE" 2>/dev/null || echo 0)
    if [ "$nprobe" -ne 1 ]; then
        echo -e "${RED}PROBE LEAK: $nprobe probes live, expected 1 — refusing to report${NC}"
        echo -e "  ${RED}gate verdicts would describe a SET of neutered gates, not this one.${NC}"
        exit 1
    fi
    commit_probe
    if ! rebuild; then
