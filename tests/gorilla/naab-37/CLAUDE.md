...
- Do NOT use `import os` or `os.system()` in Python polyglot blocks — governance blocks dangerous calls
- Do NOT use blocked languages (Go, Rust, C++, etc.) — only python, javascript, shell are allowed
- Do NOT use `agent.send()` — no API keys are available in this environment
```
...
### What to Build
A security incident triage system that:
1. Reads incident data (hardcoded JSON dicts in main.naab — NOT from external files)
2. Classifies incidents by severity using NAAb pattern matching
3. Generates analysis reports using `codegen.run_with_args` with Python variable bindings
4. Runs concurrent analysis via `async` functions
5. Sanitizes all output before printing results

### Module Specifications

**src/models.naab** — Data structures
- Exports: `make_incident(id, timestamp, source, message, severity)` — returns a dict with those keys
- Exports: `make_report(incident_id, category, findings, score)` — returns a dict with those keys
- Both return dicts with documented keys
- No imports needed besides `use json` if doing JSON serialization

**src/validators.naab** — Sanitization (REQUIRED by taint tracking)
- Exports: `sanitize_string(s)` — strips control characters and dangerous patterns, returns safe string
- Exports: `validate_report(report)` — checks required keys exist (incident_id, category, findings, score), returns bool
- Exports: `sanitize_output(text)` — prepares text for safe output (strips nulls, trims)
- These functions MUST have names starting with `sanitize_` or `validate_` for taint tracking compliance

**src/analyzer.naab** — Analysis engine
- Exports: `analyze_incident(incident)` — takes an incident dict, returns analysis dict with {category, severity_score, keywords_found, recommendation}
- Uses `codegen.run_with_args("python", python_code, bindings_dict)` to compute keyword frequency
  - Binding keys MUST be valid identifiers: `[A-Za-z_][A-Za-z0-9_]*`
  - Example: `codegen.run_with_args("python", "print(len(text.split()))", {"text": incident.get("message")})`
- Has `try/catch` for error recovery on non-governance runtime errors (e.g., missing keys)
- Exports: `analyze_batch_async(incidents)` — uses `async fn` to process multiple incidents concurrently, returns array of results

**src/main.naab** — Entry point
- Imports: `use json`, `use array`, `use string`, `use codegen`
- Imports: `import "models.naab" as models`, `import "validators.naab" as validators`, `import "analyzer.naab" as analyzer`
- Creates 3-5 test incidents with realistic security data (auth failures, port scans, malware detection, etc.)
- Runs sequential analysis on first incident
- Runs async batch analysis on remaining incidents
- Sanitizes all results with `validators.sanitize_output()` before printing
- Handles errors via try/catch (catching runtime errors, NOT governance errors)
- Prints results in a structured format showing category, severity, and recommendations

### govern.json Rules (already configured)
...
- Dangerous calls (os.system, subprocess, etc.): HARD blocked

### Codegen Variable Binding Rules
- `codegen.run_with_args(language, code, bindings_dict)` — run code with variable bindings
- Binding keys MUST be valid identifiers: `[A-Za-z_][A-Za-z0-9_]*`
- Example: `codegen.run_with_args("python", "print(count)", {"count": 42})`
- Shell bindings are safely single-quoted — pass strings freely
- Governance scans the FULL assembled code (preamble + user code) including injected binding values
- Do NOT put dangerous code in binding values — governance will block it

### What NOT to Do
- Do NOT use `codegen.run()` without args — always use `codegen.run_with_args()` with a bindings dict
- Do NOT use `import os` / `os.system()` / `subprocess` in Python polyglot or codegen blocks
- Do NOT write to files without sanitizing first (taint tracking will block)
- Do NOT use blocked languages (Go, Rust, C++, C#, Nim, Ruby, PHP, Zig, Julia)
- Do NOT use `agent.send()` — no API keys available in this test environment
- Do NOT import from absolute paths — use relative imports only
- Do NOT declare variables at top level — all `let`/`const` go inside `main {}` or functions
