...
- **Content-aware CDD** (semantic governance): CDD has **20 signals** total. Fingerprints include SHA-256 hash of response content (first 200 chars), breaking mechanical "ARAS" pattern. Response keywords extracted via `extractKeywords()` (which filters >3-char words, lowercased, with `kStopWords` set removing ~55 English function words + LLM boilerplate — no programming terms) flow through RuntimeEvent.content_keywords → CDD signals. Same stop word set in `extractKeywordsLocal()` (behavioral_sequence.cpp) for CDD plan step extraction. Content-aware CDD signals:
  ...
  - All CDD signals default enabled (S8-S20). Surfaced in: environment dict, SEMANTIC_TURN telemetry, transcript CDD section, response `semantic` section. S19 additionally produces `RECONCILIATION_TURN` telemetry and dashboard line. Environment dict includes `claim_mismatch_count`, `claim_accuracy`, `prompt_compliance_count`. Can be individually disabled via `context_drift.signals.<name>: false` in govern.json
