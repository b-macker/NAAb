...
- Continuous governance (6 features):
  ...
  - **Step-up challenges**: at elevated governance levels, inject challenge prompt and score response (word count + keyword overlap with system_prompt AND recent user prompts). Pass recovers coherence, fail blocks send. Lease renewal gated on coherence floor. Probationary lease: after `step_up_max_consecutive_failures` (default 3) consecutive fails, skip 1 challenge so actual prompt goes through — breaks death spiral where expired lease → challenge fail → turn never advances. Config: `circuit_breaker.step_up_enabled`, `step_up_at_level`, `step_up_challenge`, `step_up_min_words`, `step_up_cooldown_turns`, `step_up_keyword_threshold`, `step_up_max_consecutive_failures`. Challenge telemetry includes response_length, output_tokens, thinking_tokens, keyword_ratio, context_prompts.
...
- **Content-aware CDD** (semantic governance): CDD fingerprints now include SHA-256 hash of response content (first 200 chars), breaking the mechanical "ARAS" fingerprint pattern that caused circular detection to fire every check turn regardless of content. `isCircular()` now only fires when the agent genuinely produces identical responses. Natural healing activates on healthy turns. Three new CDD signals:
  ...
  - Configs: `signals.response_quality`, `signals.thinking_collapse`, `weights.response_quality` (0.08), `weights.thinking_collapse` (0.06), `thresholds.response_quality_min_ratio`, `thresholds.thinking_collapse_ratio`, `thresholds.thinking_history_window`
