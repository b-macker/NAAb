...
- Continuous governance (6 features):
  ...
  - **Step-up challenges**: at elevated governance levels, inject challenge prompt and score response (word count + keyword overlap with system_prompt AND recent user prompts). Pass recovers coherence, fail blocks send. Lease renewal gated on coherence floor. Failed challenges advance the turn counter (a failed challenge IS a turn — prevents death spiral where turn never advances). Config: `circuit_breaker.step_up_enabled`, `step_up_at_level`, `step_up_challenge`, `step_up_min_words`, `step_up_cooldown_turns`, `step_up_keyword_threshold`. Challenge telemetry includes response_length, output_tokens, thinking_tokens, keyword_ratio, context_prompts.
...
- **Content-aware CDD** (semantic governance): CDD fingerprints now include SHA-256 hash of response content (first 200 chars), breaking the mechanical "ARAS" fingerprint pattern that caused circular detection to fire every check turn regardless of content. `isCircular()` now only fires when the agent genuinely produces identical responses. Natural healing activates on healthy turns. Response keywords extracted via `extractKeywords()` flow through RuntimeEvent.content_keywords → CDD signals → existing dynamic systems (adaptive baseline, graduated response, reality checkpoint). Five content-aware CDD signals:
  - `response_quality`: fires when content/output token ratio < `response_quality_min_ratio` (0.3) — thinking dominates output
  - `thinking_collapse`: fires when rolling-window mean of thinking tokens drops below `thinking_collapse_ratio` (0.5) of baseline mean — progressive model degradation
  - `semantic_stability` (F19): Jaccard similarity between consecutive response keyword sets. Fires when similarity < `semantic_stability_min_overlap` (0.25) — detects abrupt topic shifts. `prev_response_keywords` stored in DriftState
  - `mandate_alignment`: rolling-window (size 20) overlap between response keywords and system_prompt keywords. Fires when mean alignment < `mandate_alignment_min` (0.15) — detects sustained drift from agent purpose. Mandate keywords initialized on first `agent.send()` via `initializeMandateKeywords()`. `mandate_alignment_history` deque in DriftState
  - Configs: `signals.response_quality`, `signals.thinking_collapse`, `signals.semantic_stability` (default false), `signals.mandate_alignment` (default false), `signals.exclude_infrastructure_errors` (default true — API/network errors bypass `repeated_failures` signal). Weights: `response_quality` (0.08), `thinking_collapse` (0.06), `semantic_stability` (0.10), `mandate_alignment` (0.12). Thresholds: `response_quality_min_ratio`, `thinking_collapse_ratio`, `thinking_history_window`, `semantic_stability_min_overlap` (0.25), `mandate_alignment_min` (0.15). Reality checkpoint: `weights.semantic_deviation` (0.0, opt-in). Pulse: subsystem 6 (semantic health). Environment dict: `semantic_stability_count`, `mandate_drift_count`, `mandate_alignment`. Response dict: `semantic` section. Telemetry: `SEMANTIC_TURN` events
