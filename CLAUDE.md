...
- **Content-aware CDD** (semantic governance): CDD has **19 signals** total. Fingerprints include SHA-256 hash of response content (first 200 chars), breaking mechanical "ARAS" pattern. Response keywords extracted via `extractKeywords()` flow through RuntimeEvent.content_keywords → CDD signals. Content-aware CDD signals:
  ...
  - All CDD signals default enabled (S8-S19). Surfaced in: environment dict, SEMANTIC_TURN telemetry, transcript CDD section, response `semantic` section. S19 additionally produces `RECONCILIATION_TURN` telemetry and dashboard line. Environment dict includes `claim_mismatch_count`, `claim_accuracy`. Can be individually disabled via `context_drift.signals.<name>: false` in govern.json
