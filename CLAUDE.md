...
- **Governance Pulse** (`src/runtime/governance_engine.cpp`): PulseVerdict (HEALTHY/DEGRADED/IMPAIRED) with hysteresis, two-phase mutex, stepped recovery, BSD emission on transitions, dashboard line. `governance.health()` stdlib returns `{verdict, coherence, governance_epoch, mode, zone, active, consecutive_passes, observation_count, integrity_verified, ...}` — it does **not** expose `governance_level`; read the level from `GOVERNANCE_LEVEL_CHANGE` telemetry (`from_level`/`to_level`) or `CDD_TURN`'s `level` field instead. Config: `governance_health` section.
